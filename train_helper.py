...
def fit_coral(epochs, model, opt, src_dl, tgt_dl, lambda_coral=1.0, loss_func=None,
              src_val_dl=None, early_stop_patience=10, use_gap=True):
    """
    Train with CORAL. New arg `use_gap` is forwarded to model(...) when requesting features.
    """
    ...
    for epoch in range(epochs):
        ...
        for (xs, ys), (xt, _) in zip(src_dl, tgt_dl):
            ...
            with autocast(enabled=amp_enabled):
                # pass use_gap to model when requesting features
                logits_s, feats_s = model(xs, return_feats=True, use_gap=use_gap)
                _, feats_t = model(xt, return_feats=True, use_gap=use_gap)
